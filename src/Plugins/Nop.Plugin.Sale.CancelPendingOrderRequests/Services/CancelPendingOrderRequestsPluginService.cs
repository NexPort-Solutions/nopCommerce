...
public class CancelPendingOrderRequestsPluginService
{
    ...
    public CancelPendingOrderRequestsPluginService(
        ...
        IRepository<LocaleStringResource> localeResourceRepository,
        ...
        ILogger logger)
    {
        ...
        _localeStringResourceRepository = localeResourceRepository;
    ...
    public async Task AddActivityLogTypesAsync()
    {
        ...
        if (!activityLogTypes.Any(x => x.SystemKeyword.Equals(PluginDefaults.EDIT_CANCELLATION_REQUEST_ACTIVITY_LOG_TYPE)))
        {
        ...
        if (!activityLogTypes.Any(x => x.SystemKeyword.Equals(PluginDefaults.DELETE_CANCELLATION_REQUEST_ACTIVITY_LOG_TYPE)))
        {
            await _activityLogTypeRepository.InsertAsync(new ActivityLogType()
            {
    ...
    public async Task AddMessageTemplatesAsync()
    {
        ...
        if (!messageTemplates.Any(x => x.Name.Equals(PluginDefaults.NEW_CANCELLATION_REQUEST_STORE_OWNER_NOTIFICATION_MESSAGE_TEMPLATE)))
        {
        ...
        if (!messageTemplates.Any(x => x.Name.Equals(PluginDefaults.NEW_CANCELLATION_REQUEST_CUSTOMER_NOTIFICATION_MESSAGE_TEMPLATE)))
        {
            await _messageTemplateService.InsertMessageTemplateAsync(new MessageTemplate
            {
        ...
        if (!messageTemplates.Any(x => x.Name.Equals(PluginDefaults.CANCELLATION_REQUEST_ACCEPTED_CUSTOMER_NOTIFICATION_MESSAGE_TEMPLATE)))
        {
            await _messageTemplateService.InsertMessageTemplateAsync(new MessageTemplate
            {
        ...
        if (!messageTemplates.Any(x => x.Name.Equals(PluginDefaults.CANCELLATION_REQUEST_REJECTED_CUSTOMER_NOTIFICATION_MESSAGE_TEMPLATE)))
        {
            await _messageTemplateService.InsertMessageTemplateAsync(new MessageTemplate
            {
    ...
    public async Task AddOrUpdateResourceAsync(string resourceName, string resourceValue, int languageId = 1)
    {
        if (await _pluginLocalizationService.CheckForExistingResourceAndAddNonExistingResource(resourceName, resourceValue, languageId))
        {
            ...
            {
                await _settingService.SetSettingAsync("Plugin.Sale.CancelPendingOrderRequests.HasModifiedLocaleResources", true);
            }
        }
    }
