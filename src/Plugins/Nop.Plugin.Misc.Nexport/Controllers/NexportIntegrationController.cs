    ...
    public async Task<IActionResult> OverrideResources(ICollection<int> selectedIds, bool allChecked)
    {
        ...
        if (selectedIds != null && selectedIds.Count != 0)
        {
            foreach (var id in selectedIds)
            {
                ...
                {
                    var nexportLocaleResources = _pluginService.GetLocaleResources();
                    var overridingResourceValue = nexportLocaleResources
                        .Where(resource => string.Equals(resource.Key, localeStringResourceById.ResourceName, StringComparison.CurrentCultureIgnoreCase))
