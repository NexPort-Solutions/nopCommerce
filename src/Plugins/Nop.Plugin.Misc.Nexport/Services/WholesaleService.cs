...
using static Nop.Plugin.Misc.Nexport.Services.WholesaleService;
using System.Runtime.CompilerServices;
using static Nop.Plugin.Misc.Nexport.Services.WholesaleService.WholesaleOrderDetails;
using static Nop.Plugin.Misc.Nexport.Services.WholesaleService.WholesaleOrderDetails.TotalsDetails;
...
public class WholesaleService : IWholesaleService
{
    ...
    private readonly IFundingPoolService _fundingPool;
    #endregion Fields
        ...
        int? fundingPoolId)
    {
        ...
        {
            var details = await Prepare(processPaymentRequest, shoppingCartItems, group, redeemByUtc, fundingPoolId);
    ...
    private async Task<Order> SaveOrderDetailsAsync(ProcessPaymentRequest processPaymentRequest, ProcessPaymentResult processPaymentResult, WholesaleOrderDetails details)
    {
        ...
        await _genericAttribute.SaveAttributeAsync(order, NexportDefaults.GROUP_MEMBERSHIP, details.MembershipGroup.OrgId);
        await _genericAttribute.SaveAttributeAsync(order, NexportDefaults.REDEEM_BY_UTC, details.RedeemByUtc);
        await _genericAttribute.SaveAttributeAsync(order, NexportDefaults.FUNDING_POOL, details.FundingPool);
    ...
    private async Task<WholesaleOrderDetails> Prepare(ProcessPaymentRequest request, IList<ShoppingCartItem> shoppingCart, OrganizationResponseItem group, DateTime? redeemBy, int? fundingPoolId)
    {
        ...
        var fundingPool = fundingPoolId is { } ? await _fundingPool.GetById(fundingPoolId.Value) : null;
    ...
    internal class WholesaleOrderDetails
    {
        ...
        internal required FundingPool ? FundingPool { get; init; }
