...
public static class Extensions
{
    ...
    public static string TruncateAtWord(this string input, int length)
    {
        ...
        return $"{input[..((nextSpaceIndex > 0) ? nextSpaceIndex : length)].Trim()}…";
    }

    public static SelectList ToNexportSelectList<TEnum>(this TEnum enumObj,
        bool markCurrentAsSelected = true, int[] valuesToExclude = null) where TEnum : struct, Enum
    {
    ...
    public static SelectList ToNexportSelectListFromEnum<TEnum>(this TEnum enumObj,
        bool markCurrentAsSelected = true, int[] valuesToExclude = null) where TEnum : Enum
    {
        var values = from TEnum enumValue in Enum.GetValues(typeof(TEnum))
            where valuesToExclude == null || !valuesToExclude.Contains(Convert.ToInt32(enumValue))
            select new { ID = Convert.ToInt32(enumValue), Name = GetDisplayName(enumValue) };
    ...
    //public static bool EnumContains<T>(this T value) where T : struct, Enum
    //{
    //    return true;
    //}

    //public static List<SelectListItem> ToEnumList<TEnum>(this TEnum enumObj) where TEnum : struct
    //{
    //    if (!typeof(TEnum).IsEnum)
    //        throw new ArgumentException("An Enumeration type is required.", nameof(enumObj));
    //}
