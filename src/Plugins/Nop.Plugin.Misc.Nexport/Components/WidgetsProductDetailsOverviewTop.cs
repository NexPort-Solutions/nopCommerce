...
public class WidgetsProductDetailsOverviewTop : NopViewComponent
{
    ...
    public async Task<IViewComponentResult> InvokeAsync(string widgetZone, object additionalData)
    {
        ...
        var model = await _nexportService.GetProductMappingByNopProductId(productDetailsModel.Id,
                (await _storeContext.GetCurrentStoreAsync()).Id) ?? await _nexportService.GetProductMappingByNopProductId(productDetailsModel.Id);

        return await ViewAsync("~/Plugins/Misc.Nexport/Views/Widget/Product/WidgetsProductDetailsOverviewTop.cshtml", model);
    }
}