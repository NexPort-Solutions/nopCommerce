...
public sealed class DashboardNotificationActionFilter : IAsyncActionFilter
{
    ...
    public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        ...
        var hasModifiedLocaleResources =
            await _settingService.GetSettingByKeyAsync("Plugin.Misc.Nexport.DiscountPerCreditHours.HasModifiedLocaleResources", false);
        ...
        var action = urlHelper.Action("EditPopup", "Plugin", new { systemName = NexportDiscountDefaults.SystemName }) +
                     "&btnId=btnRefreshList&formId=plugins-form-local";

        var warningTemplate = await _localizationService.GetResourceAsync("Plugins.Misc.Nexport.DiscountPerCreditHours.Errors.ModifiedLocaleResources");
