...
public class ArchwayEmployeeRegistrationFieldController : BasePluginController
{
    ...
    public async Task<IActionResult> OverrideResources(ICollection<int> selectedIds, bool allChecked)
    {
        ...
        if (selectedIds != null && selectedIds.Count != 0)
        {
            foreach (var id in selectedIds)
            {
                ...
                {
                    var archwayLocaleResources = _archwayPluginService.GetLocaleResources();
                    var overridingResourceValue = archwayLocaleResources
    ...
    {
        var httpPostedFile = Request.Form.Files.ToList().FirstOrDefault();
